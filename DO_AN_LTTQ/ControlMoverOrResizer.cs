...
namespace DO_AN_LTTQ
{
    internal class ControlMoverOrResizer
    {
        ...
        internal static void Init(Control control, Control container)
        {
            ...
            control.MouseUp += (sender, e) => Mouse_Release(control, e);
        }
        ...
        private static void UpdateMouseEdgeProperties(Control control, Point mouseLocationInControl)
        {
            ...
            MouseIsInRightEdge = Math.Abs(mouseLocationInControl.X - control.Width+3) <= 2;
            MouseIsInTopEdge = Math.Abs(mouseLocationInControl.Y) <= 2;
            MouseIsInBottomEdge = Math.Abs(mouseLocationInControl.Y - control.Height+3) <= 2;
        }
        ...
        private static void Mouse_Release(Control control, MouseEventArgs e) 
        {
            if (MouseIsInLeftEdge)
            {
                if (MouseIsInTopEdge)
